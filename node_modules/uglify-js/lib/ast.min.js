...
function DEFNODE(g, h, c, a) {
	...
	var b = "return function AST_" + g + "(props){ if (props) { ";
	for (var d = h.length; --d >= 0;) {
		b += "this." + h[d] + " = props." + h[d] + ";"
	}
	var e = a && new a;
	if (e && e.initialize || (c && c.initialize)) {
		b += "this.initialize();"
	}
	b += "}}";
	var j = new Function(b)();
	if (e) {
		j.prototype = e;
		j.BASE = a
	}
...
AST_Node.warn = function (a, b) {
	if (AST_Node.warn_function) {
		AST_Node.warn_function(string_template(a, b))
	}
};
...
function walk_body(a, b) {
	if (a.body instanceof AST_Statement) {
		a.body._walk(b)
	} else {
		a.body.forEach(function (c) {
			c._walk(b)
		})
	}
}
...
var AST_Toplevel = DEFNODE("Toplevel", "globals", {
	...
	wrap_enclose: function (c) {
		...
		var f = "(function(" + d.join(",") + "){ '$ORIG'; })(" + b.join(",") + ")";
		f = parse(f);
		f = f.transform(new TreeTransformer(function e(g) {
			if (g instanceof AST_Directive && g.value == "$ORIG") {
				return MAP.splice(a.body)
			}
		}));
		return f
	},
	wrap_commonjs: function (c, d) {
		var b = this;
		var a = [];
		if (d) {
			b.figure_out_scope();
			b.walk(new TreeWalker(function (g) {
				if (g instanceof AST_SymbolDeclaration && g.definition().global) {
					if (!find_if(function (h) {
		...
		var f = "(function(exports, global){ global['" + c + "'] = exports; '$ORIG'; '$EXPORTS'; }({}, (function(){return this}())))";
		f = parse(f);
		f = f.transform(new TreeTransformer(function e(h) {
			if (h instanceof AST_SimpleStatement) {
				h = h.body;
				if (h instanceof AST_String) {
					switch (h.getValue()) {
						case"$ORIG":
							return MAP.splice(b.body);
						case"$EXPORTS":
							var g = [];
							a.forEach(function (i) {
								g.push(new AST_SimpleStatement({
									body: new AST_Assign({
										left: new AST_Sub({
											expression: new AST_SymbolRef({name: "exports"}),
											property: new AST_String({value: i.name})
										}), operator: "=", right: new AST_SymbolRef(i)
									})
								}))
							});
							return MAP.splice(g)
...
var AST_Seq = DEFNODE("Seq", "car cdr", {
	...
	to_array: function () {
		...
		while (c) {
			b.push(c.car);
			if (c.cdr && !(c.cdr instanceof AST_Seq)) {
				b.push(c.cdr);
				break
			}
			c = c.cdr
	...
	add: function (b) {
		var c = this;
		while (c) {
			if (!(c.cdr instanceof AST_Seq)) {
				var a = AST_Seq.cons(c.cdr, b);
				return c.cdr = a
			}
			c = c.cdr
		}
...
TreeWalker.prototype = {
	_visit: function (c, b) {
		...
		var a = this.stack;
		var c = a.length, b = a[--c];
		while (c > 0) {
			var d = a[--c];
			if ((d instanceof AST_If && d.condition === b) || (d instanceof AST_Conditional && d.condition === b) || (d instanceof AST_DWLoop && d.condition === b) || (d instanceof AST_For && d.condition === b) || (d instanceof AST_UnaryPrefix && d.operator == "!" && d.expression === b)) {
				return true
			}
			if (!(d instanceof AST_Binary && (d.operator == "&&" || d.operator == "||"))) {
				return false
			}
			b = d
		}
	}, loopcontrol_target: function (c) {
		var b = this.stack;
		if (c) {
			for (var d = b.length; --d >= 0;) {
				var a = b[d];
				if (a instanceof AST_LabeledStatement && a.label.name == c.name) {
					return a.body
				}
			}
		} else {
			for (var d = b.length; --d >= 0;) {
				var a = b[d];
				if (a instanceof AST_Switch || a instanceof AST_IterationStatement) {
					return a
				}
			}
		}
