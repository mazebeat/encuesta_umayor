$(document).ready(function () {
	...
	$("#btn_neg").click(function (a) {
		a.preventDefault();
		$(".politicas").remove();
		var b = $(this).parents("div.alert");
		b.find("h4").remove();
		b.find("p").remove();
		b.append('<p><i class="fa fa-check fa-fw"></i>Gracias por tu tiempo, ¡Tu opinión es muy importante!</p>');
		b.removeClass("alert-warning").addClass("alert-success");
		$.get("logout", function (c) {
			if (c == "OK") {
				setTimeout('window.location.href="http://www.umayor.cl/";', 5000)
			}
		})
	});
	$("#modal1_ok").click(function (a) {
		a.preventDefault();
		$("#modal1").modal("toggle");
		$.get("add_exception", function (b) {
			if (b == "OK") {
				setTimeout('window.location.href="http://www.umayor.cl/";', 5000)
			}
		})
...
function onFormError(b) {
	b.preventDefault();
	$("html, body").animate({scrollTop: $(".errors").offset().top}, 1000);
	var a = '<div class="errors alert alert-danger">' + $username + ", necesitamos respuesta a todas tus preguntas</div>";
	$(".errors").append(a)
}
...
$("input[type=radio]").iCheck({
	...
	labelHover: true,
	cursor: true
}).on("ifChecked", function (b) {
	b.preventDefault();
	var c = $(this).attr("name");
	var a = $(this).val();
	$('select[name="' + c + '"]').select2("val", a);
	$("#survey_form").bootstrapValidator("revalidateField", c)
});
$("select").select2({
	width: "100%",
	containerCssClass: "",
	...
	var c = $(this).attr("name");
	var a = b.val;
	$('input[type=radio][name="' + c + '"][value=' + a++ + "]").iCheck("toggle");
	$("#survey_form").bootstrapValidator("revalidateField", c)
});